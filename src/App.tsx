...
function App() {
  // useAsyncData 请求用户列表数据
  const { data, isLoading } = useAsyncData<IData>(async () => {
    const response = await fetch("https://api.github.com/users");
    const rows = await response.json();
    // 控制请求时长，查看 Spinning 组件动画效果
    // const rows = await new Promise<User[]>((resolve) => {
    //   setTimeout(() => {
    //     resolve([
    //       {
    //         login: "mojombo",
  ...
  // 监听 useAsyncData 的 loading 状态变化
  useEffect(() => {
    ...
    <>
      ...
      {data?.rows.map((item) => {
        return (
          <div key={item.id}>
            {item.login}
            <img width="20" height="20" src={item.avatar_url} />
          </div>
        );
      })}
      <hr />
      <div>
        <p>Value: {value}</p>
        <button onClick={() => setValue("newValue" + getRandomValue())}>
          Update Value
        </button>
      </div>
      <hr />
      <div>Devtools' console will show</div>
      <div> 🚀🚀🚀🚀🚀Component is unmounted</div> OR
